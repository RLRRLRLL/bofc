	...
	class Slider {
		...
		setBoundaries() {
			if (window.innerWidth >= window.innerHeight) {
				...
				this.boundaries = {
					max:
						-1 * this.options.element.clientWidth +
						window.innerWidth,
					min: 0,
				...
				this.boundaries = {
					max:
						-1 * this.options.element.clientHeight +
						window.innerHeight,
					min: 0,
		...
		translateSlider(translation) {
			translation = Math.floor(translation * 200) / 200
	...
	class WebGLSlider extends Slider {
			...
			for (var i = 0; i < planeElements.length; i++) {
				...
				if (plane) {
					...
					plane.onReady(function () {
						...
						currentPlane.htmlElement
							.closest('.plane-wrapper')
						...
						anime({
							...
							update: function () {
								...
								currentPlane.uniforms.opacity.value =
									opacity.value
							}
						})
					})
		...
		setupShaderPass() {
			...
			if (this.shaderPass) {
				...
				var image = new Image()
				image.src =
					'https://www.martin-laxenaire.fr/medium/medias/skylines-displacement.jpg'
				...
				this.shaderPass.onRender(function () {
					...
					self.shaderPass.uniforms.offset.value[secondaryDirection] =
						self.shaderPass.uniforms.offset.value[
							secondaryDirection
						] + 1
				})
			}
		...
		updateMousePosUniform(mousePosition) {
			...
			if (this.shaderPass) {
				...
				var relativeMousePos = this.shaderPass.mouseToPlaneCoords(
					mousePosition[0],
					mousePosition[1]
				)
				this.shaderPass.uniforms.mousePos.value = [
					relativeMousePos.x,
					relativeMousePos.y
				]
			}
		...
		onTranslation() {
			...
			var planeTranslation = {
				x:
					this.direction === 0
						? this.translation - this.previousTranslation.x
						: 0,
				y:
					this.direction === 1
						? this.translation - this.previousTranslation.y
						: 0
			}
			...
			for (var i = 0; i < this.planes.length; i++) {
				...
				this.planes[i].setRelativePosition(
					planeTranslation.x,
					planeTranslation.y
				)
			}
			...
			if (this.shaderPass) {
				...
				var offset =
					(((this.direction - 1) * 2 + 1) * this.translation) /
					this.boundaries.referentSize
